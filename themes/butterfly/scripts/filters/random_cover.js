...
hexo.extend.generator.register('post', locals => {
  ...
  const handleImg = data => {
    ...
    const getFirstContentImage = content => {
      if (!content) return ''
      ...
      return htmlImage && htmlImage[1] ? htmlImage[1] : ''
    }
    ...
    // If cover is not set, prefer the first image in the post, then use a default cover.
    if (!coverVal) {
      const firstImage = getFirstContentImage(data.raw || data.content || data._content)
      if (firstImage) {
